...
export const TntIcons = () => {
  ...
  div.innerHTML = dataTNT.tntIcons
    .map((icon) => {
      return (
...
TntIcons.parameters = {
  docs: {
    description: {
      story:
        "For icons of <b>SAP-icons-TNT</b> font-family add the `sap-icon-TNT--{icon-name}` modifier class identifying the icon style to the `sap-icon-TNT` class. For example, for an <i>exceptions</i> icon, add `sap-icon-TNT sap-icon-TNT--exceptions` with the desired font size value.",
    },
  },
